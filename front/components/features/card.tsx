...
export function Card({ shopName, businessHours, parse, explanation }: Props) {
  ...
  return (
    ...
    >
      <div className="rounded-2 bg-white p-18"></div>
      <div className="w-full flex flex-col gap-1">
        ...
        <div className="w-full flex gap-x-4 items-center justify-between">
          ...
          <div className="bg-beige rounded-full p-1 text-black text-sm">
            {parse}
          </div>
        </div>
        <div className="text-black text-md text-start">{explanation}</div>
      </div>
    </div>
  );
}
